var content;
var selectedPhoto = {};
var fieldLabels = [ "firstName", "lastName", "gender", "gender", "birthYear",
		"ethnicity", "citizenship", "maritalStatus", "children", "smoke",
		"smoke", "hasORwantsHijab", "relocate", "relocateWhere", "education",
		"occupation", "comments", "email", "email", "mobilePhoneNumber",
		"homePhoneNumber", "pointOfContact", "city", "province", "country",
		"prefMaritalStatus", "prefMaritalStatus", "prefEthnicity",
		"prefEducation", "prefCountry", "prefAgeMin", "prefAgeMax",
		"prefComments" ];
...
function cleanSpecialChars(str) {
	...
	//return str.replace(/['"]+/g, '').replace(/[^\w\s]/gi, '').replace(/\\n/g,"\\n").replace(/\\'/g, "\\'").replace(/\\"/g, '\\"').replace(/\\&/g, "\\&").replace(/\\r/g, "\\r").replace(/\\t/g, "\\t").replace(/\\b/g, "\\b").replace(/[\/]/g, '\\/').replace(/\\f/g,"\\f").replace(/\x18/g, "").replace(/\x0B/g, '');
}
...
function submitApplicant() {
	...
	if (content[1].value == "" || content[2].value == ""
			|| content[5].value == "" || content[6].value == ""
			|| content[8].value == "" || content[9].value == ""
			|| content[19].value == "" || content[23].value == ""
			|| content[25].value == "") {
		greyBorders();
	...
	} else if (isNaN(content[5].value) || content[5].value < 1930
			|| content[5].value > (new Date().getFullYear()) - 12) {
		greyBorders();
		content[5].style.border = "1px solid red";
		...
		toast("Wrong value for some of the inputs.");
			...
			&& jQuery.isEmptyObject(selectedPhoto)) {
